...
void ComStateMachineMaster::init(LedStateMachine *ledStateMachine, EventBroker *eventBroker)
{
...
{
	int status = esp_now_send(address[deviceId], (uint8_t *)&payload, sizeof(payload));
...
void ComStateMachineMaster::update()
{
	...
	case ComStateMaster::SEND_APP:
	{
		...
		payload.payloadType = PayloadType::APP;
		if (esp_now_send(receivedPayload.macAddress, (uint8_t *)&payload, sizeof(payload)) == 0)
		{
